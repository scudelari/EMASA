...
namespace BaseWPFLibrary.Bindings
{
            ...
            EventHandler handler = (sender, eventArgs) =>
            {
                ...
                if (method != null)
                {
                    ...
                    for (int i = 0; i < arguments.Length; i++)
                    {
                        ...
                        else if (arguments[i] is string && parameters[i].ParameterType != typeof(string))
                        {
                            // The original value provided for this argument was a XAML string so try to convert it
                            arguments[i] = TypeDescriptor.GetConverter(parameters[i].ParameterType).ConvertFromString((string)arguments[i]);
                        }
    ...
    public class Int32ParExtension : MarkupExtension
    {
        public Int32ParExtension(string stringValue)
        {
        ...
        public override object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            {
                //if (serviceProvider is ServiceProviderContext)
                throw new XamlParseException($"Could not parse the given XAML value of {StringValue} into an integer.");
            }
        }
    }
